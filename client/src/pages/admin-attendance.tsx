...
export default function AdminAttendance() {
  ...
  // Reusing the same API endpoint for efficiency, filtered by date range
  ...
  const presentCount = new Set(filteredRecords.map(r => r.userId + r.date)).size; // Approximate unique attendance days
  ...
  return (
    <div className="p-6 md:p-8 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         ...
         <BentoCard 
            title="Status" 
            value={currentDate.getMonth() === new Date().getMonth() ? 'Active Month' : 'Past Month'} 
            icon={History} 
            variant="amber"
